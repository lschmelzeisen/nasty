...
class Hashtag:
    ...
    text: str  # e.g. "brexit"
    indices: Tuple[int, int]  # e.g. (16,22)
...
class UserMention:
    ...
    screen_name: str  # e.g. OHiwi-2
    id_str: str  # e.g. "1117712996795658241"
    indices: Tuple[int, int]  # e.g. (14, 21)
...
class TweetURLMapping:
    """The TweetURLMapping class. Got the short url, the url and the displayed url + indices"""
    url: str  # e.g. "https:\/\/t.co\/Dw84m8xRGw" | short t.co link
    expanded_url: str  # e.g. "http:\/\/google.com" | the whole unchanged link
    display_url: str  # e.g. "google.com" | the displayed part of the whole unchanged link
    indices: Tuple[int, int]  # e.g. (18,41)

    def __init__(self, url: str, expanded_url: str, display_url: str, indices: Tuple[int, int]):
        self.url = url
...
class Tweet:
    ...
    Got all information we can currently get from extracting it out of html data.
    That is: creation date, id, the text, hashtags, user mentions, urls,
    ...
    @classmethod  # ATM we dont need this def, since __init__ has optional arguments
    def build_from_html(cls, html_data: str, save_to: str = None) -> List["Tweet"]:
        """
...
def to_dict_list(entities: List) -> List[Dict]:
    """Collects every entity of one type and form a list of dicts. Manly for toString cases."""
    temp = []
...
def parse_html(html_data: str) -> str and List[Tweet]:
    ...
    It filters for the old/mobile twitter site and collects the following entries:
    - date
    - id
    - text
    - author and his @tag
