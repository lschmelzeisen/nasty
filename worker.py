...
# The default worker. Takes the jobs, generated by generator.py, to download and save them
def work(job: Dict) -> None:
    """Takes a job from jobs.json and executes it."""
    keyword = job["keyword"]
    ...
    Tweet.extract(f"https://mobile.twitter.com/search?q={keyword}"
                  f"%20since%3A{since}"
                  f"%20until%3A{until}"
                  f"%20lang%3A{lang}&src=typed_query", f"out/{keyword}-{since}.json.gz",
                  job)


def try_failed(job: Dict) -> None:
    """Takes a failed job, from failedJobs.json, and executes it."""
    keyword = job["keyword"]
    since = job["start_date"]
    until = job["end_date"]
    lang = job["lang"]
...
def test_failed_jobs() -> None:
    """Check's if a job is not yet done and was not collected to failedJobs. Corrects failedJobs."""
    with open("jobs.json") as jobs_file:
        for job in jobs_file:
            job = json.loads(job)
            keyword = job["keyword"]
            since = job["start_date"]
            path = Path(f"out/{keyword}-{since}.json.gz")
            if not path.is_file():
                with open("failedJobs.json") as failed_jobs_file:
                    for fail in failed_jobs_file:
                        fail = json.loads(fail)
                        if fail == job:
                            break
                    else:
                        with open("failedJobs.json", "a") as correct_fails:
...
def run(process_count: int = 1, work_mode: bool = True) -> None:
    ...
    with Pool(processes=process_count) as pool:
        if work_mode:
            with open("jobs.json") as to_do_file:
                tasks = []
            ...
            test_failed_jobs()
            with open("failedJobs.json") as failed_file:
                tasks = []
