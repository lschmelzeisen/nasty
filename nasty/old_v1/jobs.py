...
from nasty._util.time_ import daterange, yyyy_mm_dd_date
...
def build_jobs(
    keywords: Iterable[str], start_date: date, end_date: date, lang: str
) -> Iterable[Job]:
...
class JobMeta:
    ...
    def from_json(cls, obj: Dict) -> "JobMeta":
        return cls(
            ...
            exceptions=[
                JsonSerializedException.from_json(e) for e in obj["exceptions"]
            ],
        )
...
def _run_job(args: Tuple[Path, Job]) -> None:
    ...
    if meta.completed_at:
        logger.debug(
            '  Job marked as already completed at "{}".'.format(
                meta.completed_at.strftime(NASTY_DATE_TIME_FORMAT)
            )
        )
        return
    ...
    if data_file.exists():
        logger.debug(
            "  Deleting previously created data file because job did " "not complete."
        )
        data_file.unlink()
...
def run_jobs(
    jobs: Iterable[Job], num_processes: int = 1, out_directory: Path = Path("out")
) -> None:
