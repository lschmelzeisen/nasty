...
class Hashtag:
    ...
    def from_json(cls, obj: Dict) -> "Hashtag":
        return cls(text=obj["text"], indices=tuple(obj["indices"]))
...
class TweetUrlMapping:
    def __init__(
        self, url: str, expanded_url: str, display_url: str, indices: Tuple[int, int]
    ):
...
class Tweet:
    ...
    def permalink(self) -> str:
        return "https://twitter.com/{}/status/{}".format(self.screen_name, self.id)
    ...
    def to_json(self) -> Dict:
        result = {
            ...
            "entities": {
                ...
                "user_mentions": [
                    user_mention.to_json() for user_mention in self.user_mentions
                ],
    ...
    def from_json(cls, obj: Dict) -> "Tweet":
        return cls(
            ...
            hashtags=[
                Hashtag.from_json(hashtag) for hashtag in obj["entities"]["hashtags"]
            ],
            user_mentions=[
                UserMention.from_json(user_mention)
                for user_mention in obj["entities"]["user_mentions"]
            ],
            urls=[TweetUrlMapping.from_json(url) for url in obj["entities"]["urls"]],
        )
