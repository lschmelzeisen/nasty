...
def perform_advanced_search(keyword: str, date: date, lang: str) -> List[Tweet]:
    ...
    while True:
        ...
        tweets_on_page, next_cursor = _extract_tweets_from_advanced_search_page(page)
    ...
    if not tweets:
        ...
        logger.debug(
            "Found no tweets for this search. " "Maybe you need to change user agents."
        )

    return tweets


def _download_advanced_search_page(
    keyword: str,
    start_date: date,
    end_date: date,
    lang: str,
    ...
    logger = getLogger(nasty.__name__)
    logger.debug(
        'Download advanced search page (keyword: "{}", time-range: '
        '"{}" to "{}", lang: "{}", next-cursor: "{}").'.format(
            keyword, start_date.isoformat(), end_date.isoformat(), lang, next_cursor
        )
    )

    url = "https://mobile.twitter.com/search?q="
    url += quote_plus(
        "{} since:{} until:{} lang:{}".format(
    ...
    with requests.Session() as session:
        ...
        user_agent = (
            "Mozilla/5.0 (Windows; U; Windows NT 6.0; it-IT; "
        ...
        if request.status_code != 200:
            raise ValueError("Unexpected status code: {}.".format(request.status_code))

    return request.text


def _extract_tweets_from_advanced_search_page(
    page: str,
    ...
    tweets = [
        _extract_tweet_from_tweet_table(t)
...
def _extract_tweet_from_tweet_table(tweet_table: Tag) -> Tweet:
    ...
    name, screen_name = _extract_author_information_from_tweet_table(tweet_table)
...
def _extract_tweet_meta_from_tweet_table(tweet_table: Tag) -> Tuple[str, datetime]:
...
def _extract_author_information_from_tweet_table(tweet_table: Tag) -> Tuple[str, str]:
    # HTML collected and tested at: 2019-07-09
    ...
    screen_name = tweet_table.find("div", class_="username").text.strip()[len("@") :]
...
def _extract_hashtags_from_tweet_table(tweet_table: Tag) -> List[Hashtag]:
    ...
    return [
        Hashtag(h.text[len("#") :], (0, 0))
        for h in tweet_table.find_all("a", class_="twitter-hashtag")
    ]


def _extract_url_mappings_from_tweet_table(tweet_table: Tag) -> List[TweetUrlMapping]:
    ...
    return [
        TweetUrlMapping(u.get("href"), u.get("data-url"), u.text, (0, 0))
        for u in tweet_table.find_all("a", class_="twitter_external_link")
    ]


def _extract_user_mentions_from_tweet_table(tweet_table: Tag) -> List[UserMention]:
    # HTML collected and tested at: 2019-07-09
    ...
    if reply_names is not None:
        for reply in reply_names.find_all("a"):
            if reply.text.startswith("@"):
                user_mentions.append(UserMention(reply.text[len("@") :], "", (0, 0)))
...
def _extract_text_from_tweet_table(
    tweet_table: Tag,
    screen_name: str,
    ...
    prepended_user_mentions = "".join(
        ...
        if (
            user_mention.screen_name != screen_name
...
def _look_up_and_set_indices(
    ...
    for user_mention in user_mentions:
        if user_mention.id:
            user_mention.indices = _indices("@" + user_mention.screen_name, full_text)
