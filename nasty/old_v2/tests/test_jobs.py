...
from nasty._util.path import TemporaryDirectoryPath, TemporaryFilePath
...
class TestJobDumpLoad(unittest.TestCase):
    ...
    def test_search_job_trump_completed_at(self):
        job = Job(
            _SearchWork(Search.Query("trump"), 10000, 100), completed_at=datetime.now()
        )
        self.assertEqual(job, Job.from_json(job.to_json()))
    ...
    def test_search_job_trump_exceptions(self):
        ...
        job = Job(_SearchWork(Search.Query("trump"), 10, 1), exception=exception)
        self.assertEqual(job, Job.from_json(job.to_json()))
...
class TestJobsDumpLoad(unittest.TestCase):
    def test_single_search_job(self):
        def run_test(*work_args, **work_kwargs) -> None:
            ...
            with TemporaryFilePath(prefix="nasty-test-", suffix=".jsonl") as temp_file:
                jobs.dump(temp_file)
        ...
        run_test(
            Search.Query("hillary", filter=Search.Query.Filter.PHOTOS, lang="de"),
            max_tweets=500,
            batch_size=1,
        )
        run_test(
            Search.Query("obama", since=date(2009, 1, 20), until=date(2017, 1, 20)),
            max_tweets=5,
            batch_size=1000,
        )
    ...
    def test_many_search_jobs(self):
        def run_test(num_jobs) -> None:
            ...
            for i in range(1, num_jobs + 1):
                jobs.add_search_job(Search.Query(str(i)), max_tweets=i, batch_size=i)
            with TemporaryFilePath(prefix="nasty-test-", suffix=".jsonl") as temp_file:
                jobs.dump(temp_file)
...
class TestSearchJobsRun(unittest.TestCase):
    ...
    def test_success_parallel(self):
        ...
        for i in range(16):
            jobs.add_search_job(
                Search.Query(
                    "trump", since=date(2019, 1, i + 1), until=date(2019, 1, i + 2)
                ),
                max_tweets=50,
            )
    ...
    def test_success_empty(self):
        ...
        with TemporaryDirectoryPath(prefix="nasty-test-") as temp_dir:
            self.assertTrue(jobs.run(temp_dir))
            self._assert_out_dir_structure(temp_dir, jobs)
            with lzma.open(temp_dir / jobs._jobs[0].data_file_name, "rb") as fin:
                data = fin.read()
    ...
    def test_exception_internal_server_error(self):
        ...
        with TemporaryDirectoryPath(prefix="nasty-test-") as temp_dir:
            ...
            with (temp_dir / jobs._jobs[0].meta_file_name).open(
                ...
                job = Job.from_json(json.load(fin))
            self.assertEqual(job.exception.type, "UnexpectedStatusCodeException")

    def _assert_out_dir_structure(
        self, out_dir: Path, jobs: Jobs, allow_empty: bool = False
    ) -> None:
        ...
        for job in jobs._jobs:
            ...
            if len(tweets) != job.max_tweets:
                logger.warning(
                    "Found only {:d} tweets which is less than "
                    "requests max_tweets = {:d}.".format(len(tweets), job.max_tweets)
                )
